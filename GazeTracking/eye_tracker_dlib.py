...
while(True):
    ...
    faces = face_model(gray_img)
    
    # rects = find_faces(img, face_model)
    
    for face in faces:
        # shape = detect_marks(img, landmark_model, rect)
        landmarks = landmark_model(image=gray_img, box=face)
        landmarks = shape_to_np(landmarks)
        
        mask = np.zeros(img.shape[:2], dtype=np.uint8)
        
        mask, end_points_left  = eye_on_mask(mask, left, landmarks)
        mask, end_points_right = eye_on_mask(mask, right, landmarks)
        ...
        mid = int((landmarks[42][0] + landmarks[39][0]) // 2)
